...
const PROJECT_MARKERS = ['CMakeLists.txt', 'Makefile', 'meson.build', 'BUILD', '*.vcxproj'];

const SEARCH_DIRS = [
  'Documents', 'Projects', 'code', 'repos', 'src', 'dev', 'Work', 'WebstormProjects',
];
...
/** Find C++ project directories under common source folders */
function discoverProjects(): string[] {
  ...
  const markerNames = PROJECT_MARKERS.filter((m) => !m.includes('*'));
  const markerGlobs = PROJECT_MARKERS.filter((m) => m.includes('*'));
  ...
  if (hasFd) {
    ...
    const nameArgs = markerNames.flatMap((m, i) => [
      ...(i > 0 ? ['-o'] : []),
      '-name', m,
    ]);
    const globArgs = markerGlobs.flatMap((g) => ['-o', '-name', g]);
...
function pickWithFzf(projects: string[]): string | null {
  const result = spawnSync('fzf', [
    ...
    '--reverse',
    '--prompt', 'C++ project ❯ ',
    '--header', 'Select a C++ project to audit',
...
async function pickWithClack(projects: string[]): Promise<string | null> {
  ...
  const selected = await clack.select({
    message: 'Select a C++ project to audit',
...
program
  .name('cpp-perf-audit')
  .description('A beautiful C++ performance & safety analysis CLI powered by Claude AI')
  .version('1.0.0')
  .argument('[path]', 'Path to C++ project directory', '.')
