...
export async function run(targetPath: string, options: CliOptions): Promise<void> {
  ...
  const scanSpinner = createSpinner('Scanning for C++ projects...');
  scanSpinner.start();
  ...
  try {
    ...
    scanSpinner.fail('No C++ projects found');
  ...
  if (projects.length === 1) {
    ...
    const selected = await clack.select({
      ...
      options: projects.map((p) => ({
        label: p.name,
        value: p.name,
        hint: `${p.cppStandard} (${p.buildSystem})`,
      })),
    });
  ...
  logStep('Project', `${project.name} (${project.cppStandard})`);
  logStep('Build', project.buildSystem);

  if (project.dependencies.length > 0) {
    logStep('Dependencies', project.dependencies.map((d) => d.name).join(', '));
  }
